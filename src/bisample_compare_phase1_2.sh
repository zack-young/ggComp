...
while read lines;do
    ...
    for chr in `echo $chr_lis`;do
        ...
        if [ "${b}" == 0 ];then
            #echo 'file zero' 
            awk '{print $1"\t"$2"\t"$3"\t"SAMPLE2 item "_CNV"}' SAMPLE2="$SAMPLE2" item="$item" ${SAMPLE2_DEV}/${chr}.mask_CNV_${item}  > ${pair_DEV}/${chr}_${SAMPLE2}to${SAMPLE1}_own_CNV_${item}
        else
            #echo 'file not zero' 
            awk 'NR==FNR{a[$2]=$0;next}NR>FNR{if($2 in a ==0) print $1"\t"$2"\t"$3"\t"SAMPLE2 item "_CNV"}' SAMPLE2="$SAMPLE2" item="$item" ${SAMPLE1_DEV}/${chr}.mask_CNV_${item} ${SAMPLE2_DEV}/${chr}.mask_CNV_${item} > ${pair_DEV}/${chr}_${SAMPLE2}to${SAMPLE1}_own_CNV_${item}
        fi
        b=`du -h ${SAMPLE2_DEV}/${chr}.mask_CNV_${item} | awk '{print $1}'`
        #echo ${b}
        if [ "${b}" == 0 ];then
            #echo 'file zero'
            awk '{print $1"\t"$2"\t"$3"\t"SAMPLE1 item "_CNV"}' SAMPLE1="$SAMPLE1" item="$item" ${SAMPLE1_DEV}/${chr}.mask_CNV_${item}  > ${pair_DEV}/${chr}_${SAMPLE1}to${SAMPLE2}_own_CNV_${item}
        else
            #echo 'file not zero'
    ...
    #3  
        ...
        cat ${pair_DEV}/${chr}.${SAMPLE1}to${SAMPLE2}all_CNV_deletion ${pair_DEV}/${chr}.${SAMPLE1}to${SAMPLE2}all_CNV_duplication | sort -u -nk2,2 > ${pair_DEV}/${chr}.${SAMPLE1}to${SAMPLE2}all_CNV
    done
done < $config
echo "Pairwise CNV detection Complete"
